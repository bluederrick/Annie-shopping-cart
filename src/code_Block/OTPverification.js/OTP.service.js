...
export const verifyOTPservice = async (data) => {
  ...
  if (!userOTPverificatopRecords.length > 0) {
    // no record found
    throw new Error(
      'No record was found for user  ${userOTPverificatopRecords} , please SIGNUP'
    );
  }
  // user otp record exist
  //   console.log(userOTPverificatopRecords[0]);
  ...
  const updatedOTP = await _User.findByIdAndUpdate(
    id,
    { verified: true, state: 'active' }
    // { state: 'active' }
  );
  console.log(updatedOTP);
  const deletedRecord = await _Otp.deleteOne({ id });
